...
#MERGE!
combineddaily.am <- bind_rows(cycle.day, am_data)
combineddaily.pm <- bind_rows(cycle.day, pm_data)
```
#Add in LH Data
```{r}
LH.Count <- read.csv("/Users/apatte21/Documents/CLEAR Lab Repositories/UKAlc_daily_cleaning/UKAlc.LH.Data.csv")

cycleday.am.merge <- merge(LH.Count, am_data, by = c("ID", "daterated"), all = T)
cycleday.pm.merge <- merge(LH.Count, pm_data, by = c("ID", "daterated"), all = T)
```
