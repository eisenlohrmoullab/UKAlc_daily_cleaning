...
```{r}
#number of people who binge drank on Sundays = 53
am_data_Sun %>% group_by(ID) %>% 
  filter(Alc_Amt_am >= 4) 
#number of people who binge drank on Mondays = 15
am_data_Mon %>% group_by(ID) %>% 
  filter(Alc_Amt_am >= 4) 
#number of people who binge drank on Tuesdays = 21
am_data_Tues %>% group_by(ID) %>% 
  filter(Alc_Amt_am >= 4) 
#number of people who binge drank on Wednesdays = 15
am_data_Wed %>% group_by(ID) %>% 
  filter(Alc_Amt_am >= 4) 
#number of people who binge drank on Thursdays = 16
am_data_Thurs %>% group_by(ID) %>% 
  filter(Alc_Amt_am >= 4) 
#number of people who binge drank on Fridays = 24
am_data_Fri %>% group_by(ID) %>% 
  filter(Alc_Amt_am >= 4) 
#number of people who binge drank on Saturdays = 38
