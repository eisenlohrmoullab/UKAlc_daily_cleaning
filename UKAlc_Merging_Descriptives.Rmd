...
#Remove rows where everything is NA except daterated 
```{r}
na_rows_am <- rowSums(is.na(am_data[, .SD, .SDcols = !c('Date', 'ID')]) > 0
na_rows_am

na_rows_pm <- rowSums(is.na(pm_data[, .SD, .SDcols = !c('Date', 'ID')]) > 0
na_rows_pm 
```
