...
### create person means and person deviations for am_data
```{r echo=T}
#create person means
...
#Create person deviations    
am_data <- am_data %>% group_by(ID) %>% mutate(Urg_Alc_am.d = sd(Urg_Alc_am, na.rm = T),
                                               Urg_Nic_am.d = sd(Urg_Nic_am, na.rm = T),
                                               Urg_Mar_am.d = sd(Urg_Mar_am, na.rm = T),
                                               Urg_Coc_am.d = sd(Urg_Coc_am, na.rm = T),
                                               Urg_Oth_am.d = sd(Urg_Oth_am, na.rm = T),
                                               Crav_Alc_am.d = sd(Crav_Alc_am, na.rm = T),
                                               Crav_Mar_am.d = sd(Crav_Mar_am, na.rm = T),
                                               Crav_Coc_am.d = sd(Crav_Coc_am, na.rm = T),
                                               Crav_Benz_am.d = sd(Crav_Benz_am, na.rm = T),
                                               Crav_Hall_am.d = sd(Crav_Hall_am, na.rm = T),
                                               Crav_Opiate_am.d = sd(Crav_Opiate_am, na.rm = T),
                                               Alc_Amt_am.d = sd(Alc_Amt_am, na.rm = T),
                                               Alc_Drunk_am.d = sd(Alc_Drunk_am, na.rm = T)) 
...
#create.deviation <- function(am_data, Urg_Alc_am, Urg_Alc_am.m,
#                              Urg_Nic_am, Urg_Nic_am.m, 
#                              Urg_Mar_am, Urg_Mar_am.m,
#                              Urg_Coc_am, Urg_Coc_am.m,
#                              Urg_Oth_am, Urg_Oth_am.m,
#                              Crav_Alc_am, Crav_Alc_am.m,
#                              Crav_Nic_am, Crav_Nic_am.m,
#                              Crav_Mar_am, Crav_Mar_am.m,
#                              Crav_Coc_am, Crav_Coc_am.m,
#                              Crav_Benz_am, Crav_Benz_am.m,
#                              Crav_Hall_am, Crav_Hall_am.m,
#                              Crav_Opiate_am, Crav_Opiate_am.m,
#                              Alc_Amt_am, Alc_Amt_am.m) {
#   am_data <- am_data %>%
#     rowwise() %>%
#     mutate("{{Urg_Nic_am}}.d" := {{Urg_Alc_am}} - {{Urg_Alc_am.m}})
# }
# 
# create.deviation(am_data, Urg_Alc_am, Urg_Alc_am.m,
#                  am_data, Urg_Nic_am, Urg_Nic_am.m,
#                  am_data, Urg_Mar_am, Urg_Mar_am.m,
#                  am_data, Urg_Coc_am, Urg_Coc_am.m,
#                  am_data, Urg_Oth_am, Urg_Oth_am.m,
#                  am_data, Crav_Alc_am, Crav_Alc_am.m,
#                  am_data, Crav_Nic_am, Crav_Nic_am.m,
#                  am_data, Crav_Mar_am, Crav_Mar_am.m,
#                  am_data, Crav_Coc_am, Crav_Coc_am.m,
#                  am_data, Crav_Benz_am, Crav_Benz_am.m,
#                  am_data, Crav_Hall_am, Crav_Hall_am.m,
#                  am_data, Crav_Opiate_am, Crav_Opiate_am.m,
#                  am_data, Alc_Amt_am, Alc_Amt_am.m)

#Do I need to do rolling average? 
...
# }
# ###I'm not sure why this below isn't making a new column? 
# am_data$Alc_Amt_am.roll <- create.3day.rolling.avg(am_data, Alc_Amt_am, 3)
# View(am_data$Alc_Amt_am.roll)
```
...
#Rolling averages - is this necessary? If so, can Jordan help walk me through the for loops? 
```{r}
...
#create varlist for for loop 
# am_data_roll <- am_data %>% 
#   select(c(Urg_Alc_am,
#            Urg_Nic_am,
#            Urg_Mar_am,
#            Urg_Coc_am,
...
#            Alc_Drunk_am))
# 
# for (i in am_data_roll) {
#   am_data_rolling <- create.3day.rolling.avg(am_data_roll, !!sym({{i}}))
# }
...
pm_data <- pm_data %>% group_by(ID) %>% mutate(Urg_Alc_pm.m = mean(Urg_Alc_pm, na.rm = T),
                                               ...
                                               Urg_Coc_pm.m = mean(Urg_Coc_pm, na.rm = T))
#create person sd's 
pm_data <- pm_data %>% group_by(ID) %>% mutate(Urg_Alc_pm.d = sd(Urg_Alc_pm, na.rm = T),
                                               Urg_Nic_pm.d = sd(Urg_Nic_pm, na.rm = T),
                                               Urg_Mar_pm.d = sd(Urg_Mar_pm, na.rm = T),
                                               Urg_Coc_pm.d = sd(Urg_Coc_pm, na.rm = T),
                                               Diff_Inhib_pm.d = sd(Diff_Inhib_pm, na.rm = T),
                                               Diff_Change_pm.d = sd(Diff_Change_pm, na.rm = T),
                                               Diff_Stop_pm.d = sd(Diff_Stop_pm, na.rm = T),
                                               Immed_Payoff_pm.d = sd(Immed_Payoff_pm, na.rm = T),
                                               Res_Urge_pm.d = sd(Res_Urge_pm, na.rm = T),
                                               Diff_LongRew_pm.d = sd(Diff_LongRew_pm, na.rm = T),
                                               Diff_ResRew_pm.d = sd(Diff_ResRew_pm, na.rm = T),
                                               Dep_pm.d = sd(Dep_pm, na.rm = T),
                                               Hopeless_pm.d = sd(Hopeless_pm, na.rm = T),
                                               Worthless_pm.d = sd(Worthless_pm, na.rm = T),
                                               Anxious_pm.d = sd(Anxious_pm, na.rm = T),
                                               MoodSwing_pm.d = sd(MoodSwing_pm, na.rm = T),
                                               RejSens_pm.d = sd(RejSens_pm, na.rm = T),
                                               Angry_pm.d = sd(Angry_pm, na.rm = T),
                                               Conflicts_pm.d = sd(Conflicts_pm, na.rm = T),
                                               Loss_Interest_pm.d = sd(Loss_Interest_pm, na.rm = T),
                                               Diff_Conc_pm.d = sd(Diff_Conc_pm, na.rm = T),
                                               Tired_pm.d = sd(Tired_pm, na.rm = T),
                                               OverEat_pm.d = sd(OverEat_pm, na.rm = T),
                                               Sp_Food_Crav_pm.d = sd(Sp_Food_Crav_pm, na.rm = T),
                                               High_Sleep_pm.d = sd(High_Sleep_pm, na.rm = T),
                                               Diff_Sleep_pm.d = sd(Diff_Sleep_pm, na.rm = T),
                                               Overwhelm_pm.d = sd(Overwhelm_pm, na.rm = T),
                                               Loss_Control_pm.d = sd(Loss_Control_pm, na.rm = T),
                                               Breast_Tend_pm.d = sd(Breast_Tend_pm, na.rm = T),
                                               Breast_Swell_pm.d = sd(Breast_Swell_pm, na.rm = T),
                                               Headache_pm.d = sd(Headache_pm, na.rm = T),
                                               JM_Pain_pm.d = sd(JM_Pain_pm, na.rm = T),
                                               Loss_Prod_pm.d = sd(Loss_Prod_pm, na.rm = T),
                                               Int_Relat_pm.d = sd(Int_Relat_pm, na.rm = T),
                                               Urg_Alc_pm.d = sd(Urg_Alc_pm, na.rm = T),
                                               Urg_Nic_pm.d = sd(Urg_Nic_pm, na.rm = T),
                                               Urg_Mar_pm.d = sd(Urg_Mar_pm, na.rm = T),
                                               Urg_Coc_pm.d = sd(Urg_Coc_pm, na.rm = T))
```
