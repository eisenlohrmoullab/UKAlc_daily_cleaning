...
subset(am_data, id == 106 & daterated == ymd("2021-05-09"))
...
#will come back to this shortly - was interrupted to run Sat afternoon errands 
result_am <- am_data %>% group_by(ID, Date) %>% summarise(observations = n())
result_pm <- pm_data %>% group_by(ID, Date) %>% summarise(observations = n())
```
