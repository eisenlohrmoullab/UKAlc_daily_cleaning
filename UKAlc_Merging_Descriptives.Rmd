...
### identify how many people drank any alcohol throughout the study 
```{r}
#attempt number 1 
unique_ID <- unique(am_data$ID)
count_above_1_or_equal <- sum(am_data$drink_today_am >= 1)
count_unique_above_1_or_equal <- sum(am_data$ID[am_data$drink_today_am >= 1] %in% unique_ID)
count_unique_above_1_or_equal
#attempt number 2 
result <- am_data %>%
  group_by(ID) %>%
  summarize(any_drinking = any(drink_today_am > 1)) %>%
  summarise(count = sum(any_drinking))

count_of_individuals_with_value_above_1 <- result$count
count_of_individuals_with_value_above_1
#attempt number 3 
result <- am_data %>%
  group_by(ID) %>%
  summarize(any_drinking = sum(am_data$drink_today_am >= 1)) %>%
  ungroup()

sum(am_data$any_drinking)
#attempt number 4 
result <- am_data %>%
  group_by(ID) %>%
  summarize(has_value_ge_1 = any(drink_today_am >= 1)) %>%
  ungroup() %>%
  summarize(count = sum(has_value_ge_1))
result

### Jordan's suggestion below 
summary_Alc_daily <- am_data %>% group_by(ID) %>% 
  ...
  arrange(drink_today_am) ## getting confused at the arrange step 

summary_Alc_daily
##COUNTS
summary_Alc_daily %>% tally() 
## 91 total pts
summary_Alc_daily %>% tally(TotalDrinkingDays ==0)
## 13 nondrinkers
summary_Alc_daily %>% tally(TotalDrinkingDays == 1 | TotalDrinkingDays == 2)
##COUNTS
summary_Alc_daily %>% tally() 
## days not drinking per person ##found this to be kind of nice, thinking about how to incorporate this into my dataset in a meaningful way? maybe I just make a new df 
am_data %>% tally(drink_today_am ==0)

## days drinking per person 
am_data %>% tally(drink_today_am >= 1)
```
...
### make list and then put list into the function instead of the variable (in this case Alc_Amt_am)


am_data <- create.3day.rolling.avg(am_data, Alc_Amt_am) 
