...
#Anna is looking into how to remove rows that are all NA except daterated and ID
=======
am_data <- am_data %>% drop_na(daterated, ID)
pm_data <- pm_data %>% drop_na(daterated, ID)

am_data <- am_data[!rowSums(is.na(am_data[, colnames(am_data) != ID, daterated])) == (ncol(am_data)
pm_data <- pm_data[!rowSums(is.na(pm_data[, colnames(pm_data) != ID, daterated])) == (ncol(pm_data)                                                                                
>>>>>>> 836b151f09db7f0810b602920303e2629f48b48e
```
