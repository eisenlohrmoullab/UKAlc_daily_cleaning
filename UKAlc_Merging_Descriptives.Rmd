...
library(ggdist)
library(ggrain)
install.packages("zoo") #for rolling avgs
library(zoo)
...
am_data <- create.3day.rolling.avg(am_data, am_data_vars_int) 
...
```{r}
#weekend means Saturday/Sunday (bc amts are for the night before)
...
ggplot(am_day_of_week, aes(x = factor(daterated), y = Alc_Amt_am)) +
  geom_point(position = position_jitter(width = 0.1), size = 1) +
  labs(x = "Day of the Week", y = "Value") +
  scale_x_discrete(labels = c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"))
ggplot(am_data, aes(x = factor(daterated), y = Alc_Amt_am)) +
  geom_boxplot() +
  labs(x = "Day of the Week", y = "Amount of Alcohol") +
  scale_x_discrete(labels = c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"))
##Tried to plot by day of week 
# set.seed(123)
# am_day_of_week <- data.frame(
#   DayOfWeek = rep(1:7, each = 10),
#   Value = rnorm(70)
# )
# ggplot(am_day_of_week, aes(x = factor(daterated), y = Alc_Amt_am)) +
#   geom_point(position = position_jitter(width = 0.1), size = 1) +
...
###Plot from Jordan's template from "ADHD-KY Plotting Across Cycle"
# group_by(am_day_of_week, Alc_Amt_am) %>%
#   ggplot(aes(x=factor(am_day_of_week, levels = Alc_Amt_am), 
#              group=Alc_Amt_am, color=Alc_Amt_am))+
#   geom_line()+
#   geom_hline(yintercept = 0, linetype="dashed", col="gray")+
#   scale_y_continuous(expand = c(0,0.005))+
#   theme_bw()+
#   xlab("Day of Week")+
#   ylab("Amount of Alcohol Consumed")+
#   labs(color = "")+
#   theme(legend.position = "bottom")
#Alc Urge 
ggplot(am_data_weekend, aes(1, Crav_Alc_am)) +
  geom_rain() +
  theme_classic() +
  theme(axis.title.x = element_blank(), 
        axis.text.x = element_blank(), axis.ticks.x = element_blank())
ggplot(am_data_weekend, aes(1, Crav_Alc_am.d)) +
  geom_rain() +
  theme_classic() +
  theme(axis.title.x = element_blank(), 
        axis.text.x = element_blank(), axis.ticks.x = element_blank())
