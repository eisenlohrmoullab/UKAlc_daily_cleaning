...
menses.data <- read.csv("/Users/apatte21/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/UKALC/02_datasets/UKALC_DAILY/01_raw_data/Period Start Dates.csv")

menses.am.merge <- merge(menses.data, am_data, by = "ID")
menses.pm.merge <- merge(menses.data, pm_data, by = "ID")

#compare df 
#Github from clear3 cleaning daily had this -> compare_df_cols(clear1daily, clear2daily, return = "mismatch") but I got an error message that compare_df_cols wasn't a function, so I did the below using setdiff instead 
diff.am.menses <- setdiff(menses.data, am_data)
diff.pm.menses <- setdiff(menses.data, pm_data)

#address mismatches
#examples from git: clear2daily$angrum <- as.numeric(clear2daily$angrum)
#clear2daily <- clear2daily %>% mutate(across(starts_with("study"),as.numeric))

#run compare df again
compare_df_cols(menses.data, am_data, return = "mismatch")
compare_df_cols(menses.data, pm_data, return = "mismatch")

#MERGE!
combineddaily.am <- bind_rows(menses.data, am_data)
combineddaily.pm <- bind_rows(menses.data, pm_data)

#delete column if all values are NA (ex: survey opened and nothing entered)
not_all_NA <- function(x) any(!is.na(x))
combineddaily.am <- combineddaily.am %>% select_if(not_all_NA)
combineddaily.pm <- combineddaily.pm %>% select_if(not_all_NA)
