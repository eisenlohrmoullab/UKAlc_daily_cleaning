...
am_data[c('Date', 'Time')] <- str_split_fixed(am_data$DateTime, ' ', 2)
pm_data[c('Date', 'Time')] <- str_split_fixed(pm_data$DateTime, ' ', 2)

df = subset(am_data, select = -c(DateTime) )
df = subset(pm_data, select = -c(DateTime))
```
