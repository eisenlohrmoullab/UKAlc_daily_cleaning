...
```{r}
#person-mean-center
cycleday_alcamt_plot1 <- cycleday.am.merge %>% group_by(ID, cycleday) %>% 
  filter(!is.na(cycleday)) %>%
  summarize(Alc_Amt_am.m=mean(Alc_Amt_am.m),
            Amt.day.m = mean(Alc_Amt_am, na.rm = TRUE),
            Amt.day.d = Amt.day.m - Alc_Amt_am.m) 

cycleday_alcamt_plot1 %>% group_by(cycleday) %>%
  summarize(personcentered.dev = mean(Amt.day.d, na.rm=T),
            se=sd(Amt.day.d, na.rm=T)/sqrt(n())) %>%
  mutate(roll= rollapply(personcentered.dev, 3, 
                         mean, align = "center", fill=NA)) %>%
  ggplot(aes(x=cycleday, y=roll))+
  geom_line(group=1)+
