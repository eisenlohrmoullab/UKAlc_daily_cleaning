...
#Create variables of interest list to put into loop function 
...
am_vars_roll_list <- c("Urg_Alc_am", "Urg_Nic_am", "Urg_Mar_am") #Finish list later 
...
am_data_hybridcycleday <- cycleday.am.merge %>%
...
am_data_cycleday <- am_data_cycleday %>%
...
am_data_cycleday <- cycleday.am.merge %>%
  group_by(ID) %>%
...
#person-mean-center
data_cycleday_plot <- am_data_cycleday %>% group_by(ID, hybridcycleday) %>% 
  filter(!is.na(hybridcycleday)) %>%
...
data_daycountLH_plot <- am_data_cycleday %>% group_by(ID, daycountLH) %>% 
