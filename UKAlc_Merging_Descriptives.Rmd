...
write.csv(pm_data, "~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/UKALC/02_datasets/UKALC_DAILY/03_cleaned_data/pm_data_cleaned.csv", row.names=TRUE)
```
...
```{r}
#Lab Visit Data 
labvisit <- read_csv("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/UKALC/02_datasets/UKALC_LABVISITS/01_raw_data/Mini Dataset 11.30.23.csv") 
```
...
```{r}
alc.visit.dates <- read_xlsx("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/UKALC/02_datasets/UKALC_DAILY/01_raw_data/Alcohol Session Dates.xlsx")
...
```{r}
labvisit.am.data.merge <- merge(labvisit_clean, am_data_clean, by = c("ID", "daterated"), all = T)
...
```{r}
library(readxl)
hormones.lab <- read_xlsx("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/UKALC/02_datasets/UKALC_HORM/02_data_prep_workspace/Salivary_Results_For_Import.xlsx")
```

#add "day in study column" for hormone data and save new csv 
```{r}
hormones.lab <- separate(hormones.lab, col = Sample, into = c("ID", "day_in_study"), sep = "-")

write.csv(hormones.lab, "~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/UKALC/02_datasets/UKALC_HORM/02_data_prep_workspace/Salivary_Hormones_StudyDays.csv", row.names=TRUE)
```
...
cycleday.am.merge <- merge(cycle.day, am_data_clean, by = c("ID", "daterated"), all = T)
cycleday.pm.merge <- merge(cycle.day, pm_data_clean, by = c("ID", "daterated"), all = T)
```
#Add in LH Data
```{r}
