...
selected_columns <- saliva_cycleday_merge_am[, c("ID", "ovpos", "E2", "P4", "LH", "firstdayofperiod", "day_in_study")]

#Create new dataframe with daily hormones and menses/ov data 
hormone_comparison <- data.frame(selected_columns)
hormone_comparison$H_E2 <- hormone_comparison$E2
...
horm_compare_long %>% 
  filter(ID==101) %>% 
ggplot(horm_compare_long, aes(x = day_in_study, y = value, color = variable)) +
  geom_line() +
  labs(x = "day in study", y = "hormone level") +
