...
#person-mean-center Urg_Alc on each specific day
...
data_daycountLH_urg_plot %>%
  group_by(daycountLH) %>%
  summarize(personcentered.dev=mean(Urg_Alc_am.day.d, na.rm=T),
            se = sd(Urg_Alc_am.day.d, na.rm=T)/sqrt(n())) %>%
  mutate(roll= rollapply(personcentered.dev, 3, 
                         mean, align = "center", fill=NA)) %>%
  filter(!is.na(daycountLH)) %>%
